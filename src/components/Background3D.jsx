...
function Geometry() { const ref = useRef(); useFrame((state) => { if (!ref.current) return; ref.current.rotation.x = state.clock.elapsedTime * .12; ref.current.rotation.y = state.clock.elapsedTime * .18; ref.current.position.y = Math.sin(state.clock.elapsedTime * .7) * .45; ref.current.position.x = 4.8 + state.pointer.x * .4; }); return <group ref={ref} position={[4.8, .5, -4]}><mesh><icosahedronGeometry args={[1.7, 2]} /><meshBasicMaterial color="#20d9c2" wireframe transparent opacity={.28} /></mesh><gridHelper args={[10, 12, "#1d827a", "#102c31"]} rotation={[Math.PI / 2.1, 0, 0]} position={[0, 0, -2]} /></group>; }
