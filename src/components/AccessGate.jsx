...
export default function AccessGate() {
  const [visible, setVisible] = useState(() => sessionStorage.getItem("affan-portfolio-access") !== "granted");
  const [entering, setEntering] = useState(false);
  useEffect(() => { if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) { sessionStorage.setItem("affan-portfolio-access", "granted"); setVisible(false); } }, []);
  const enter = () => { setEntering(true); window.setTimeout(() => { sessionStorage.setItem("affan-portfolio-access", "granted"); setVisible(false); }, 850); };
